...
const Form = () => {
  ...
  const getItem = async () => {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_ENDPOINT}/api/forms?populate=deep`,
      {
        method: "GET",
        headers: {
                        ...
                        >
                          ...
                          <div className="flex justify-between w-full px-4 box-border items-center sm:pl-8">
                            ...
                            <div className="flex flex-wrap justify-between w-24 md:w-40 lg:w-64 gap-y-2 ml-[2%]">
                              <Link
                                href={`${process.env.NEXT_PUBLIC_API_ENDPOINT}${form?.URL?.data?.attributes?.url}`}
                              >
                              ...
                              {form?.WordURL?.data ? (
                                <div className="bg-[#efefef] rounded-xl text-xs sm:text-base p-2  px-4 sm:px-6 py-2">
                                  <Link
                                    href={`${process.env.NEXT_PUBLIC_API_ENDPOINT}${form?.WordURL?.data?.attributes?.url}`}
                                    download={true}
                                  >
                                    Word
                                  </Link>
